...
def desktop_auto_login(request):
    """Auto-login endpoint for Electron desktop app (no credentials needed)."""
    from users.models import User
    from rest_framework_simplejwt.tokens import RefreshToken
    user, _ = User.objects.get_or_create(
        email='desktop@localhost',
        defaults={'role': 'ADMIN', 'first_name': 'Desktop', 'last_name': 'User',
...
urlpatterns = [
    ...
    path('api/desktop-auth/', desktop_auto_login, name='desktop_auto_login'),
    ...
    # Milvus URL removed — replaced by embedded ChromaDB
    ...
    # 🦙 Ollama Local LLM Management API
    path('api/ollama/', include('agent_orchestration.ollama_urls')),
    
    # 💬 In-App Chatbot API (chat with workflows)
    path('api/chatbot/', include('agent_orchestration.chatbot_urls')),

    # 🤖 PHASE 3: Agent Orchestration DocAware API
    path('api/agent-orchestration/', include('agent_orchestration.workflow_urls')),
    
    # Workflow Deployment API removed for Electron desktop version.
    ...
    # Public chatbot API removed for Electron desktop version.
